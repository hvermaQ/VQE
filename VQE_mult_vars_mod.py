...
#contour_vals = np.arange(0.05, 0.5, 0.05, dtype=float)
contour_vals = np.array([0.01])
#contour_vals = np.linspace(0.05, 1, 10, dtype=float)
...
def main():
    ...
    for err in eps_arr:
        ...
        for fixed_metric in selected_metrics:
            ...
            indices = np.where(np.isclose(metric_arr, fixed_metric, atol=1e-2))
...
def plot_results(res):
    ...
    for eps_err, data in res.items():
        ...
        plt.plot(metrics, efficiencies, '-o', label=r'$\epsilon = %s \times 10^{-%s}$'%(base_expo[count][0],base_expo[count][1]))
        count+=1
    plt.xlabel(r'$ E(N_{it}, N_g) - E_{gs}$')
    plt.ylabel(r'$log(Max (\eta))$')
    ...
    for eps_err, data in res.items():
        ...
        metrics = data[:, 0]
        resources = data[:, 2]
        plt.plot(resources, metrics, '-x', label=r'$\epsilon = %s \times 10^{-%s}$'%(base_expo[count][0],base_expo[count][1]))
        count+=1
    plt.ylabel(r'$ E(N_{it}, N_g) - E_{gs}$')
    plt.xlabel(r'$ Min(N_{it}~\times N_g)$')
    #plt.yscale('log')
    #plt.title(r'$Min(N_{it \times N_g}) vs Fixed Metric$')
    plt.legend()
    plt.grid()
    plt.show()
    return
